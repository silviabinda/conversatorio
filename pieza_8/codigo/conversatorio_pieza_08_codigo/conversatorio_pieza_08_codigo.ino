...
On the display we can read following text in Spanish language:
"The enactment of the Spanish Constitution, on 6 December '78, represents the legal affirmation of equality between women and men and the desire to eradicate discrimination based on race, sex or religion, as well as their recognition as the main inspiration for Spain's legal system.
...
Complete list of board 8 components:
1) 19cm x 19cm custom designed perforated board, 3D printed with black PLA fillament
2) Custom designed holder for OLED display, 3D printed with black PLA fillament
3) Custom designed holder for ESP32 microcontroller, 3D printed with black PLA fillament
...
GPIOs for connections:
  ...
  RGB LED module:
    ...
    G -> GPIO 13
    B -> GPIO 26
   (NOTE: In this board we decided not to use the red color of the RGB LED module.)
________________________________________________________________________________________________________________
...
char message1[] = "La promulgacion de la Constitucion Espanola, el 6 de diciembre del '78, representa la afirmacion legal de la igualdad entre mujeres y hombres, y el deseo de erradicar la discriminacion basada en la raza, el sexo o la religion, asi como su reconocimiento como la principal inspiracion para el sistema legal de Espana.";

char message2[] = "Hasta septiembre de 2023, se han registrado 78 feminicidios en Espana a lo largo de este ano. A pesar de que Espana tiene un indice de igualdad de genero del 78%, todavia es necesario avanzar para lograr la paridad de genero.";
...
const int greenPin = 13; // Green LED connected to digital pin 13
...
void loop() {
  ...
  // Call the function to control the LED dimming
  rgb_led_dim();
}
